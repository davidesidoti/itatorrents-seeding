...
def _cache_path() -> Path:
    try:
        from . import config
        return Path(config.runtime_setting("ITA_LANG_CACHE_PATH", default=str(Path.home() / ".itatorrents_lang_cache.json")))
    except Exception:
        return Path(os.environ.get("ITA_LANG_CACHE_PATH") or (Path.home() / ".itatorrents_lang_cache.json"))
