...
All subsequent steps run as `itatorrents` unless stated otherwise.
...
Create `/etc/systemd/system/itatorrents.service`:
...
Create `/opt/itatorrents/itatorrents.env` (mode 600):
...
!!! note "Subpath vs root"
    The example serves at the domain root. If you want a subpath (e.g. `itatorrents.example.com/itatorrents/`), add a trailing slash to `proxy_pass` (`proxy_pass http://127.0.0.1:8765/;`) so nginx **strips** the prefix → leave `ITA_ROOT_PATH=""`. Or keep nginx non-stripping (no trailing slash) and set `ITA_ROOT_PATH=/itatorrents`.
...
Files to back up periodically (only user `itatorrents` needs access):

```
/opt/itatorrents/itatorrents.env               # secrets
~/Unit3Dup_config/Unit3Dbot.json               # tracker + client config
~/.itatorrents_db.json                         # upload history
~/.itatorrents_tmdb_cache.json                 # regeneratable
~/.itatorrents_lang_cache.json                 # regeneratable
```
...
- [ ] `https://itatorrents.example.com` responds, login works
- [ ] `journalctl -u itatorrents -f` → no errors
